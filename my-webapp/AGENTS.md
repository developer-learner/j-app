...
| 2026-06-26 | Agent crossed role boundaries in interactive mode — prior conversation history from a different agent carried over, and the agent complied instead of refusing. Architect edited public/ files, PM deployed, etc. | G-05: Each agent prompt now has a ROLE BOUNDARY header that tells the agent to ignore prior context from other roles and refuse cross-boundary requests. Coupled with INV-2 gate (`bash scripts/phase-gate.sh <role>`). |
